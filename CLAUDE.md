...
- Main app with two subcommands: `silo` (API queries) and `local` (NetCDF files)
...
### Module Dependencies
- CLI imports `silo_api`, `read_silo_xarray`, and `download_silo` modules
...
- Download command requires year range, not date range (uses --start-year/--end-year)
